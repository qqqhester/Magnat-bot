...
from keyboards.inline import get_b2b_shop_main
...
async def show_shop_main(event: Message | CallbackQuery) -> None:
    """Единая точка отрисовки главного меню магазина (работает и с командами, и с колбэками)."""
    ...
    if isinstance(event, CallbackQuery):
        await event.message.edit_text(text, reply_markup=get_b2b_shop_main())
    else:
        await event.answer(text, reply_markup=get_b2b_shop_main())
    ...
    for item_id, info in ITEMS_CONFIG.items():
        if user_level >= info["level"]:
            ...
            builder.row(InlineKeyboardButton(
                ...
                callback_data=f"buy_select:{item_id}:1"
            ))
        else:
...
async def shop_execute_deal(callback: CallbackQuery) -> None:
    ...
    # Бесшовное возвращение в главное меню без спама сообщениями
...
async def shop_price_config_init(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        f"Введите желаемую розничную стоимость за 1 единицу (число).\n\n"
        "<i>Внимание: Если цена превысит рыночную более чем на 20%, вы начнете терять авторитет.</i>"
    )
